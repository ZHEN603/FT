...
export function QuoteDetail({ quote, onChanged, onMessage }: { quote: QuoteWithItems; onChanged: () => Promise<void>; onMessage: (message: string) => void }) {
  ...
  return (
    <aside className="admin-detail quote-detail">
      ...
      <DetailSection title="4. 产品明细（部分）">
        <table className="mini-items-table"><tbody>{quote.items.slice(0, 3).map((item) => <tr key={item.id}><td><input type="checkbox" /></td><td>{item.name}</td><td>{item.sku}</td><td>{item.quantity}</td><td>{usd.format(item.amount)}</td></tr>)}</tbody></table>
      </DetailSection>
